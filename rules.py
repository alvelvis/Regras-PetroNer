...
def log():
    import os, sys, re
    fname = sys.argv[1]
    skip_lines = sys.argv[3] if len(sys.argv) > 3 else ""
    ...
    os.system("python3 rules_fake.py '{}' '{}'; rm rules_fake.py".format(fname, sent_id if sent_id else ""))
    if 1: exit()
log()
# end log
...
def change_and_log(token, col, value, line):
    skip_lines = []
    if str(line) in skip_lines:
        return token.__dict__[col] # not change value if line should be skipped
...
with open("rules-log.txt" if not test_sent_id else "rules-log_test.txt", "a") as f:
    f.write("{} rules were applied\n\n{}".format(len(rules), json.dumps(sorted([(x,y) for x,y in rules.items()], key=lambda x: -x[1]))))
...
plt.title('Número de Entidades por Classe (Total: %s)' % sum(dados.values()))
